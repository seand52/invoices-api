import { Controller, Post, Body, UseGuards, UsePipes, ValidationPipe, Request, Get, Patch } from '@nestjs/common';
...
export class BusinessInfoController {
  ...
  async createBusinessInfo(@Body() businessInfoData: CreateBusinessInfoDto, @Request() req: any): Promise<BusinessInfo  | {}> {
    const {userId} = req.user;
  ...
  @Patch()
  @UsePipes(ValidationPipe)
  async updateBusinessInfo(@Body() businessInfoData: UpdateBusinessInfoDto, @Request() req: any): Promise<string> {
    const {userId} = req.user;
    return this.businessInfoService.updateBusinessInfo(businessInfoData, userId);
  }
